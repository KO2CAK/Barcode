...
export function GridScan({
  ...
  const motion = useMemo(() => {
    ...
    return {
      ...
      depthResponse: THREE.MathUtils.lerp(0.25, 0.45, s),
    ...
    const onClick = async () => {
      ...
      if (
        ...
        try {
          ...
          // no permission prompt result needed
        }
      }
    };
    const onEnter = () => {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const skew = new THREE.Vector2(
        lookCurrent.current.x * motion.skewScale,
        -lookCurrent.current.y * motion.yBoost * motion.skewScale
      );
      material.uniforms.uSkew.value.set(skew.x, skew.y);
      material.uniforms.uTilt.value = tiltCurrent.current * motion.tiltScale;
      material.uniforms.uYaw.value = THREE.MathUtils.clamp(
        yawCurrent.current * motion.yawScale,
        -0.6,
        0.6
      );
  ...
  useEffect(() => {
    if (!enableGyro) return undefined;
    const handler = (e) => {
      if (uiFaceActive) return;
        ...
        if (ts - lastDetect >= 33) {
          ...
          try {
            ...
            if (res && res.detection) {
              ...
              const depthScale = 1 + motion.depthResponse * (faceSize - 0.25);
  ...
  }, [enableWebcam, modelsReady, motion.depthResponse]);
  ...
  return (
    ...
    >
      {(showPreview || enableWebcam) && (
        <div className={showPreview ? "gridscan__preview" : "sr-only"}>
